...
export async function day21a(data: string[]) {
  ...
  const getMinDistForOutputChange = ({
    ...
    const distanceMap = MAPS[keypadIndex].distances;
    const out: Output = {
      keypadIndex,
      input,
      dist: 0,
    ...
    if (out.children.length > 0) {
      ...
      out.dist = out.outputs.reduce((sum, path) => sum + path.length, 0);
    }
    return out;
  };
  ...
  const configMap = new Map<string, { dist: number; path: Path['path'] }>();
  for (let fromIndex = 0; fromIndex < OUTPUT_CONFIGS.length; fromIndex++) {
    const fromConfig = CONFIGS[fromIndex];
    for (let toIndex = 0; toIndex < OUTPUT_CONFIGS.length; toIndex++) {
      if (fromIndex === toIndex) {
        continue;
      }
      const toConfig = CONFIGS[toIndex] as Path['path'];

      const keypadIndex = MAPS.length - 1;
      console.log(
        '###################################################################',
        fromConfig,
        toConfig
      ...
      const getPath = (out: Output) => {
        if (out.outputs) {
          return out.outputs.flat();
        }
        return out.children.map(getPath).flat();
      };
      const path: Path['path'] = getPath(out);
  ...
  for (const code of data) {
    ...
    for (const char of code) {
      ...
      const { dist, path } = configMap.get(key)!;
      fullDist += dist;
      fullPath += path.join('');

      currentConfig = toConfig;
    }
    console.log(`${code}: ${fullPath} (${fullDist})`);
