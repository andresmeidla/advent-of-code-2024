...
export async function day16a(data: string[]) {
  ...
  const dijkstra = (
    ...
    const visitedKey = (gn: { node: Node }) => `${gn.node.x},${gn.node.y}`;
    ...
    while (minQueue.size() > 0) {
      ...
      if (edge.node === end) {
        return { edge, totalDistance, path };
      }
  ...
  const { path: pathNodes, totalDistance } = dijkstra(
    nodeMap,
    startingNode,
    end
  );
  ...
  for (const edge of pathNodes) {
    console.log(edge.node, edge.dir, edge.distance);
