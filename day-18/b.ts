...
export async function day18a(data: string[]) {
  ...
  function dijkstra(
    ...
    while (minQueue.size() > 0) {
      openSet.sort((a, b) => a.dist - b.dist); // Sort to get the node with the lowest g score
      ...
      for (const [dx, dy] of neighbors) {
        ...
        if (
          ...
          const tentativeGScore =
            (gScore.get(current.position.toString()) ?? Infinity) + 1;
          if (
            tentativeGScore < (gScore.get(neighborPos.toString()) ?? Infinity)
          ) {
            const neighborNode: Node = {
              position: neighborPos,
              dist: tentativeGScore,
              parent: current,
            };
            gScore.set(neighborPos.toString(), tentativeGScore);
